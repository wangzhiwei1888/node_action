...
class MyPromise extends EventEmitter {
    ...
    _handle(deferred) {
        ...
        // 立即resolve的Promise，原生ES6的实现：是在本轮事件循环结束时，而不是在下一轮事件循环的开始时。
        //TODO: MyPromise的实现是在下一轮事件循环的开始时，是否有何潜在的问题
        this._setImmediate(() => {
    ...
    _setImmediate(fn) {
        let asap = (typeof setImmediate === 'function' && setImmediate) || (fn => setTimeout(fn, 0));
        asap(fn);
    }
